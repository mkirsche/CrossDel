...
echo 'aligned reads'
delete=`java -cp "${BINDIR}" BestDeletionFinder $OUTDIR/alignment_of_assembled_reads/"$x"_all.sam.$c $x $offset 'SEQ'`
pos=`java -cp "${BINDIR}" BestDeletionFinder $OUTDIR/alignment_of_assembled_reads/"$x"_all.sam.$c $x $offset 'POS'`
